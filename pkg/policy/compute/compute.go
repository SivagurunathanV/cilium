...
func (r *IdentityPolicyComputer) processRequests(ctx context.Context) error {
	...
	for {
		...
		for i := range results {
			if results[i].res.Err != nil {
				// The identity was removed before its policy was computed.
				// Retrying spins forever since no further delete event will
				// arrive to drop the request. Endpoint regen re-reads statedb
				// and recovers.
